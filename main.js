...
array.map((object, index) => {
  domString += 
      `<div id="newkid" class="card mb-3"  style="max-width: 340px;">
        <div class="row g-0">
...
const renderToJail = (array) => {
  ...
  array.map((object, index) => {
    domStringJail += 
        `<div id="darkside" class="card m-1" style="width: 15rem;" id="${index}">
        ...
        <div class="card-body text-center">
          <p class="h6 card-text text-dark">Oh dang... <span class="text-danger bold">${object.name}</span> joined the dark side!</p>
        </div>
      </div>`
      })
